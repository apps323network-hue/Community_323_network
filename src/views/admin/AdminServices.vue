<template>
  <AdminLayout>
    <div class="w-full flex flex-col gap-6 sm:gap-8">
        ...
        <div class="bg-surface-dark rounded-xl p-6 border border-white/10 hover:border-white/20 transition-all">
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="p-2 bg-white/5 rounded-lg">
              <span class="material-symbols-outlined text-white/50 text-xl">business_center</span>
            </div>
          </div>
          <div class="text-4xl font-black text-white mb-1">{{ serviceStats.total }}</div>
          <div class="text-xs text-white/40">Serviços cadastrados</div>
        </div>

        <div class="bg-surface-dark rounded-xl p-6 border border-white/10 hover:border-white/20 transition-all">
          <div class="flex items-center justify-between mb-4">
            <span class="text-white/70 text-sm font-medium">Ativos</span>
            <div class="p-2 bg-white/5 rounded-lg">
              <span class="material-symbols-outlined text-white/50 text-xl">check_circle</span>
            </div>
          </div>
          <div class="text-4xl font-black text-white mb-1">{{ serviceStats.active }}</div>
          <div class="text-xs text-white/40">Serviços ativos</div>
        </div>

        <div class="bg-surface-dark rounded-xl p-6 border border-white/10 hover:border-white/20 transition-all">
          <div class="flex items-center justify-between mb-4">
            <span class="text-white/70 text-sm font-medium">Inativos</span>
            <div class="p-2 bg-white/5 rounded-lg">
              <span class="material-symbols-outlined text-white/50 text-xl">cancel</span>
            </div>
          </div>
          <div class="text-4xl font-black text-white mb-1">{{ serviceStats.inactive }}</div>
          <div class="text-xs text-white/40">Serviços inativos</div>
        </div>

        <div class="bg-surface-dark rounded-xl p-6 border border-white/10 hover:border-white/20 transition-all">
          <div class="flex items-center justify-between mb-4">
            <span class="text-white/70 text-sm font-medium">Em Destaque</span>
            <div class="p-2 bg-white/5 rounded-lg">
              <span class="material-symbols-outlined text-white/50 text-xl">star</span>
            </div>
          </div>
          <div class="text-4xl font-black text-white mb-1">{{ serviceStats.featured }}</div>
          <div class="text-xs text-white/40">Serviços em destaque</div>
        </div>
      </div>
      ...
      <div v-else-if="displayedServices.length === 0" class="flex flex-col items-center justify-center py-12 bg-surface-dark/50 rounded-xl border border-white/10">
        <span class="material-symbols-outlined text-gray-500 text-6xl mb-4">business_center</span>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-2">
                <h3 class="text-white font-bold text-lg">{{ service.nome }}</h3>
                <span
              ...
              <p v-if="service.categoria" class="text-white/60 text-sm mb-2">
                {{ service.categoria }}
              </p>
              <p v-if="service.descricao" class="text-white/40 text-sm line-clamp-2">
                {{ service.descricao }}
              </p>
            </div>
          </div>
          ...
          <div v-if="service.beneficio_membro" class="mb-4 p-3 rounded-lg bg-secondary/10 border border-secondary/20">
            ...
            <p class="text-sm text-gray-300">{{ service.beneficio_membro }}</p>
          </div>

          <div class="flex gap-2 mt-4">
            <button
              @click="editService(service)"
              class="flex-1 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white text-sm font-medium transition-all"
            >
              Editar
            </button>
            <button
      ...
      >
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-white mb-2">Parceiro</label>
              <input
          ...
          <div>
            <label class="block text-sm font-medium text-white mb-2">Benefício para Membros</label>
            <input
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-white mb-2">Preço (em centavos)</label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-white mb-2">Moeda</label>
              <select
          ...
          <div class="flex gap-4">
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span class="text-sm text-white">Em destaque</span>
            </label>
