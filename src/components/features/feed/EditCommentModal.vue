...
watch(() => props.modelValue, (newVal) => {
  if (newVal && props.comment) {
    ...
    content.value = props.comment.conteudo
    error.value = null
  } else {
    content.value = ''
    error.value = null
...
async function handleSave() {
  if (!props.comment || !content.value.trim() || loading.value) return
  ...
  try {
    await editComment(props.comment.id, content.value.trim())
    toast.success(t('posts.commentEdited') || 'Comentário editado com sucesso!')
    emit('saved')
    isOpen.value = false
  } catch (err: any) {
    error.value = err.message || t('posts.editError') || 'Erro ao editar comentário'
