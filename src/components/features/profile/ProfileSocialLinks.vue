  ...
  <div class="bg-surface-dark/80 backdrop-blur rounded-2xl p-6 border border-input-border hover:border-primary/30 transition-colors">
    ...
    <div class="space-y-4">
      ...
      <div class="group/link">
        <div v-if="linkedin && (!isEditingLinkedin || readonly)" class="flex items-center gap-3 p-3 rounded-lg bg-input-bg border border-transparent hover:border-secondary/50 hover:shadow-[0_0_10px_rgba(0,240,255,0.1)] transition-all">
          <svg class="w-5 h-5 opacity-70 group-hover/link:opacity-100" viewBox="0 0 24 24" fill="currentColor">
            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
          </svg>
          <span class="text-sm text-text-muted group-hover/link:text-white flex-1 truncate transition-colors">{{ formatSocialLink(linkedin || '') }}</span>
          <button v-if="!readonly" @click="isEditingLinkedin = true" class="text-text-muted hover:text-white transition-colors">
      ...
      <!-- Instagram -->
      <div class="group/link">
        <div v-if="instagram && (!isEditingInstagram || readonly)" class="flex items-center gap-3 p-3 rounded-lg bg-input-bg border border-transparent hover:border-primary/50 hover:shadow-[0_0_10px_rgba(255,0,153,0.1)] transition-all">
          <svg class="w-5 h-5 opacity-70 group-hover/link:opacity-100" viewBox="0 0 24 24" fill="currentColor">
          ...
          <span class="text-sm text-text-muted group-hover/link:text-white flex-1 truncate transition-colors">{{ formatSocialLink(instagram || '') }}</span>
          <button v-if="!readonly" @click="isEditingInstagram = true" class="text-text-muted hover:text-white transition-colors">
            <span class="material-symbols-outlined text-[16px]">edit</span>
          </button>
        </div>
        <div v-else-if="!readonly" class="relative group/input">
